...
export function AgentRoster({ className }: { className?: string }) {
  ...
  return (
    <aside className={cn("w-full h-full bg-[#0d0f12] flex flex-col border-none", className)}>
      <div className="p-3 border-b border-white/5">
        <h2 className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em]">Employee Roster</h2>
      </div>
            ...
            return (
              ...
              >
                <div className="relative mt-0.5">
                  <div
                    className={cn(
                      ...
                      isSelected
                        ...
                        : agent.isActive
                        ? "bg-[#1e2430] border-white/10"
                        : "bg-[#1e2430] border-white/5 opacity-50"
                    )}
                  >
                  ...
                  <div
                    className={cn(
                      "absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-[#0d0f12]",
                      isWorking
                        ? "bg-amber-400 animate-pulse"
                        : agent.isActive
                        ? "bg-primary shadow-[0_0_5px_rgba(0,201,167,0.5)]"
                        : "bg-[#484f58]"
                    )}
                  />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className={cn(
                      ...
                      isSelected ? "text-primary" : "text-[#e2e8f0]"
                    )}>
                      {agent.name}
                    </span>
                  </div>
                  ...
                  {isWorking && (
                    <Badge variant="outline" className="mt-1 text-[10px] border-amber-500/30 bg-amber-500/10 text-amber-400 py-0 uppercase font-bold tracking-tighter">
                      Working
                    </Badge>
                  )}
                </div>
                <button
                  ...
                  className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity p-1 rounded-md hover:bg-white/5 text-[#8b949e] hover:text-[#e2e8f0]"
                >
      ...
      <div className="p-3 border-t border-white/5 space-y-2">
        <Button
          ...
          className="w-full bg-primary/10 hover:bg-primary/20 text-primary border border-primary/20 text-[10px] uppercase font-bold tracking-widest h-8"
        >
