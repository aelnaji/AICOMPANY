...
export function ChatPanel() {
  ...
  if (!isChatOpen || !selectedAgentId) return null;

  const isWorking = isAgentWorking[selectedAgentId];

  return (
    <aside className="w-96 shrink-0 border-l border-border bg-[#0d1117] flex flex-col">
      {/* Chat Header */}
      <div className="flex items-center justify-between p-3 border-b border-border">
        <div className="flex items-center gap-2.5">
          <div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#22c55e]/20 border border-[#22c55e]/30">
            {ROLE_ICONS[agentInfo?.role || ""] || <Bot className="h-4 w-4 text-[#22c55e]" />}
          </div>
          <div>
        ...
        <div className="flex items-center gap-1.5">
          {isWorking && (
            <Badge variant="outline" className="text-[10px] border-amber-500/30 bg-amber-500/10 text-amber-400 py-0 gap-1">
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3">
        {messages.length === 0 && (
          <div className="flex flex-col items-center justify-center h-full text-center text-[#484f58]">
            ...
            <p className="text-xs mt-1">Send a message to {agentInfo?.name}</p>
          </div>
        )}
        {messages.map((msg) => (
          <div
          ...
          >
            <div
              className={cn(
                ...
                msg.role === "user"
                  ? "bg-[#22c55e]/15 border border-[#22c55e]/20 text-[#e2e8f0]"
                  : "bg-[#161b22] border border-[#21262d] text-[#e2e8f0]"
              )}
            >
              {msg.role === "assistant" && (
                <div className="flex items-center gap-1.5 mb-1.5">
                  ...
                  <span className="text-[10px] text-[#8b949e] font-medium">{agentInfo?.name}</span>
                </div>
              )}
        ...
        {(sending || isWorking) && (
          <div className="flex justify-start chat-message-enter">
            <div className="bg-[#161b22] border border-[#21262d] rounded-lg px-3.5 py-2.5">
              ...
              <div className="flex gap-1.5 py-1">
                <span className="typing-dot h-2 w-2 rounded-full bg-[#22c55e]" />
                <span className="typing-dot h-2 w-2 rounded-full bg-[#22c55e]" />
                <span className="typing-dot h-2 w-2 rounded-full bg-[#22c55e]" />
              </div>
            </div>
      ...
      <div className="p-3 border-t border-border">
        <div className="flex items-end gap-2">
          <Textarea
            ...
            className="min-h-[40px] max-h-[120px] resize-none bg-[#161b22] border-[#21262d] text-[#e2e8f0] placeholder:text-[#484f58] text-sm focus:border-[#22c55e]/50"
            rows={1}
          />
          <Button
            onClick={handleSend}
            disabled={!input.trim() || sending}
            className="h-10 w-10 shrink-0 bg-[#22c55e] hover:bg-[#16a34a] text-black"
          >
